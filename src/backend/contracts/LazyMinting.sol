...
contract LazyMinting is ERC721URIStorage, EIP712, AccessControl {
    ...
    function lazyMint(
        ...
        require(voucher.signerAddress == isValid, "Failed to mint");
        tokenId++;
        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, voucher.uri);
        emit handleLazyMint(voucher.signerAddress, msg.sender, voucher.uri, tokenId);
        return tokenId;
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(address signerAddress,string title,string description,string userId,string uri)"
                        ),
